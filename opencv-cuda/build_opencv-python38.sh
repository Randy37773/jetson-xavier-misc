...
readonly PREFIX=~/.local  # RG: USER-INSTALL
...
main () {
    ...
    echo "PREFIX: _${PREFIX}_ _$PREFIX_"
    if [[ -w ${PREFIX} ]] ; then
        make install 2>&1 | tee -a install.log
    else
	echo "you should NOT SEE THIS. this should be avoided"
	echo "avoid a sudo make install (and root owned files in ~) if $PREFIX is writable"
